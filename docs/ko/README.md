...
ULW는 VS Code 기본 터미널 대신 **사이드바**에 터미널 MUX(`native`, `tmux`, `zellij`)를 띄웁니다. OpenCode 등 AI CLI는 선택 사항이며, 터미널에서 직접 실행하면 됩니다.
...
- 뷰가 열리면 설정한 **백엔드**(일반 셸 / `tmux` / `zellij`)만 시작 — AI 툴 선택 UI 없음
...
3. `ulw.autoStartOnOpen`으로 백엔드를 자동 시작하거나 수동으로 시작합니다.
4. 셸·멀티플렉서를 쓰고, 필요할 때 OpenCode 등을 직접 실행합니다.
...
| 설정                           | 설명                                                                           |
| ------------------------------ | ------------------------------------------------------------------------------ |
| `ulw.autoStart`                | 뷰 활성화 시 터미널 세션 자동 시작                                              |
| `ulw.autoStartOnOpen`          | 사이드바 열 때 터미널 시작 (`ulw.terminalBackend` 기준)                        |
...
| `ulw.contextDebounceMs` | 컨텍스트 업데이트 debounce 지연 |

### 백엔드 및 인스턴스 탐색

| 설정                  | 설명                                                              |
| --------------------- | ----------------------------------------------------------------- |
| `ulw.terminalBackend` | 사이드바 시작 시 `native` / `tmux` / `zellij`                     |
| `ulw.enableAutoSpawn` | 프로세스 스캔 시 OpenCode 미발견이면 백그라운드 실행(사이드바 UI와 무관) |
